...
function Nathalie_Mota_scripts_styles()
{
   ...
   */
	...
	wp_localize_script('ajax-script', 'ajax_object', array('ajax_url' => admin_url('admin-ajax.php'),'pic_data' => get_pic_data()));
...
function my_acf_blocks_init()
{
	...
	if (function_exists('acf_register_block_type')) {
		acf_register_block(array(
			'name'             => 'banner',
			'title'            => __('Banner', 'Nathalie_Mota'),
			'description'      => __('Banner', 'Nathalie_Mota'),
			'render_callback'  => 'Nathalie_Mota_render_callback',
			'category'         => 'layout',
			'mode' 						 => 'auto',
			...
			'example' => array(
				'attributes' => array(
					'mode' => 'preview',
					'data' => array(
						'preview_image_help' => get_template_directory_uri() . '/images/preview-acf/preview-banner.png',
					),
				),
			),
		));
		...
		acf_register_block(array(
			'name'             => 'photos',
			'title'            => __('Photos', 'Nathalie_Mota'),
			'description'      => __('Photos', 'Nathalie_Mota'),
			'render_callback'  => 'Nathalie_Mota_render_callback',
			'category'         => 'layout',
			'mode' 						 => 'auto',
			'icon'             => 'dashicons dashicons-block-default',
			...
			'example' => array(
				'attributes' => array(
					'mode' => 'preview',
					'data' => array(
						'preview_image_help' => get_template_directory_uri() . '/images/preview-acf/preview-photos.png',
					),
				),
			),
		));
...
/**
 * Add function for filter photos.
 *
 * @since Nathalie Mota 0.1
 */
...
function filter_photos()
{
	...
	$all_zone = [];
	$all_categorie = [];
	foreach ($photo_query->posts as $photo) {
		$type = get_field('photo-type', $photo);
		if ($type && !empty($type)) {
			$photo->type = $type;
		...
		$categories = wp_get_post_terms($photo->ID, 'categorie');
		...
		$formats = wp_get_post_terms($photo->ID, 'format');
		...
		$photo->link = get_permalink($photo->ID);
		if ($image = get_the_post_thumbnail_url($photo->ID, 'blog')) {
			$photo->image = $image;
		} else {
			$photo->image = '';
		}

		$reference = get_field('type-reference', $photo->ID);
		if ($reference) {
	...
	$output['all_photos'] = $all_photos;
	$output['all_type'] = $all_type;
	$output['all_categorie'] = $all_categorie;
	$output['all_format'] = $all_format;
...
function get_pic_data()
{
	...
	// while ($photo_query->have_posts()) {
		...
		if ( get_field( 'type-reference',$id) ){
			$ref = get_field( 'type-reference',$id);
		}
		$data[$url] = ['ref'=> $ref, 'cat' => $categorie];
	}
